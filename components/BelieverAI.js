...
const TOOLS = [
  {
    ...
    label: "Humanize",
    icon: PenLine,
    blurb: "Rewrite stiff AI prose into something that sounds like you wrote it.",
    placeholder: "Paste the AI-generated text you'd like to humanize…",
    cta: "Humanize this",
  ...
  },
  {
    ...
    label: "Detect",
    icon: ScanSearch,
    blurb: "Estimate how likely a passage is to be machine-written.",
    placeholder: "Paste text to check for signs of AI authorship…",
    cta: "Analyze text",
    endpoint: "/api/detect",
  },
  {
    ...
    label: "Improve",
    icon: GraduationCap,
    blurb: "Get line-edits and feedback on an essay draft, like a margin full of notes.",
  ...
  },
  {
    ...
    label: "Translate",
    icon: Languages,
    blurb: "Translate text into another language.",
    placeholder: "Type, paste, or speak the text you'd like to translate…",
    cta: "Translate",
...
function RuledTextarea({ value, onChange, placeholder, onMicResult }) {
  ...
  return (
    <div className="relative rounded-sm overflow-hidden" style={{ border: `1px solid ${TOKENS.paperLine}` }}>
      <div
        ...
        style={{ borderRight: `1px solid ${TOKENS.crimsonSoft}`, opacity: 0.5 }}
      />
      <textarea
        value={value}
        onChange={onChange}
      ...
      {supported && (
        <button
          ...
          style={{
            background: listening ? TOKENS.crimson : TOKENS.ink,
            color: TOKENS.paper,
          }}
        >
  ...
  return (
    <div className="w-full">
      ...
      <div
        ...
        style={{ background: `linear-gradient(90deg, ${TOKENS.sage}, ${TOKENS.crimsonSoft}, ${TOKENS.crimson})` }}
      >
        <div
          className="absolute -top-1.5 w-1 h-5 rounded-full"
          style={{ left: `${score}%`, background: TOKENS.ink, transform: "translateX(-50%)" }}
        />
      </div>
    </div>
  );
...
export default function BelieverAI() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const output = await callTool(tool.endpoint, body);
      if (active === "humanize") setHumanizeOut(output);
      if (active === "detect") setDetectOut(output);
      if (active === "improve") setImproveOut(output);
      if (active === "translate") setTranslateOut(output);
    } catch (e) {
      ...
      <div className="w-full max-w-2xl">
        <div className="flex items-center gap-3 mb-6 px-1">
          ...
          <div>
            <h1
              className="text-2xl"
              style={{ fontFamily: "'Fraunces', serif", fontStyle: "italic", fontWeight: 600, color: TOKENS.paper }}
            >
              BelieverAI
            </h1>
        ...
        <div className="p-6 rounded-b-md rounded-tr-md" style={{ background: TOKENS.paper }}>
          ...
          {error && (
            <p className="mt-4 text-sm" style={{ color: TOKENS.crimson, fontFamily: "'IBM Plex Mono', monospace" }}>
              {error}
            </p>
          )}
          ...
          {active === "humanize" && humanizeOut && (
            <div className="mt-6 pt-5" style={{ borderTop: `1px dashed ${TOKENS.paperLine}` }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => copyText(humanizeOut)}
                  className="flex items-center gap-1 text-xs"
                  style={{ color: TOKENS.sage, fontFamily: "'IBM Plex Mono', monospace" }}
                >
              ...
              <p
                ...
                style={{ color: TOKENS.charcoal, fontFamily: "'Fraunces', serif" }}
              >
                {humanizeOut}
              </p>
            </div>
          ...
          {active === "detect" && detectOut && (
            <div className="mt-6 pt-5 space-y-4" style={{ borderTop: `1px dashed ${TOKENS.paperLine}` }}>
              ...
              <Stamp
                tone={detectOut.verdict === "likely human" ? "human" : detectOut.verdict === "likely ai" ? "ai" : "mid"}
              >
                {detectOut.verdict}
              </Stamp>
              <ul className="space-y-1.5">
                {(detectOut.notes || []).map((n, i) => (
                  <li
                    key={i}
                    className="text-sm leading-6 flex gap-2"
                    style={{ color: TOKENS.charcoal, fontFamily: "'Fraunces', serif" }}
                  >
                    <span style={{ color: TOKENS.crimson }}>—</span>
                    {n}
                  </li>
                ))}
              </ul>
          ...
          {active === "improve" && improveOut && (
            <div className="mt-6 pt-5 space-y-4" style={{ borderTop: `1px dashed ${TOKENS.paperLine}` }}>
              <p className="text-sm leading-6 italic" style={{ color: TOKENS.charcoal, fontFamily: "'Fraunces', serif" }}>
                {improveOut.overall}
              </p>
              <div className="space-y-3">
                {(improveOut.suggestions || []).map((s, i) => (
                  <div key={i} className="pl-3" style={{ borderLeft: `2px solid ${TOKENS.crimsonSoft}` }}>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: TOKENS.muted, fontFamily: "'IBM Plex Mono', monospace" }}>
                      {s.reason}
                    </p>
                  </div>
                ))}
